...
def _serialize(
    *,
    promo: core.PromoCode | None,
    spec: dict[str, Any],
    already_used: bool,
    timestamp: datetime,
    effective_uses: int | None = None,
) -> dict[str, Any]:
...
async def limited_promo_status(
    slug: str,
        ...
        if promo is not None:
            ...
            already_used = session.scalar(
                select(core.PromoRedemption.id).where(
                    ...
                    core.PromoRedemption.telegram_id == tg_user["id"],
                )
            ) is not None
...
async def limited_promo_activate(
    slug: str,
    x_telegram_init_data: str | None = Header(default=None, alias="X-Telegram-Init-Data"),
):
    tg_user = core.verify_init_data(x_telegram_init_data or "")
        ...
        with core.SessionLocal() as session:
            with session.begin():
                ...
                promo = session.scalar(
                    select(core.PromoCode)
                    .where(core.PromoCode.code == spec["code"])
                    .with_for_update()
                )
                if promo is None:
                    raise HTTPException(status_code=404, detail="Промо не найдено")
                ...
                result = {
                    ...
                    "transaction": core.serialize_transaction(tx),
                    "promo": _serialize(
                        promo=promo,
                        spec=spec,
                        already_used=True,
                        timestamp=timestamp,
                        effective_uses=promo.uses_count,
                    ),
                }
