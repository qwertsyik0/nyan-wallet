...
REGISTER_NAME = "register_maintenance_handlers"
...
def backup(path: Path, stamp: str) -> Path | None:
    if not path.exists():
        return None
    target = path.with_name(f"{path.name}.before_maintenance_{stamp}.bak")
...
def has_import(tree: ast.Module) -> bool:
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "maintenance_bot":
...
def patch_bot(source: str) -> str:
    ...
    if not has_import(tree) or not has_registration(tree):
        raise InstallError("Не удалось безопасно подключить maintenance handler")
...
def download_module() -> bytes:
    request = urllib.request.Request(
        MODULE_URL,
        headers={"User-Agent": "Nyan-Wallet-Maintenance-Installer/1.0"},
    )
    try:
        with urllib.request.urlopen(request, timeout=20) as response:
            data = response.read()
    except Exception as exc:
        raise InstallError(f"Не удалось скачать maintenance_bot.py: {exc}") from exc

    try:
        source = data.decode("utf-8")
        compile(source, "maintenance_bot.py", "exec")
    except Exception as exc:
        raise InstallError(f"Загруженный maintenance_bot.py повреждён: {exc}") from exc
    return data
...
def run_checks(python: Path, root: Path, files: list[Path]) -> None:
    ...
    smoke = subprocess.run(
        [
            str(python),
            "-c",
            "import maintenance_bot; assert callable(maintenance_bot.register_maintenance_handlers); print('OK')",
        ],
        cwd=root,
        text=True,
        capture_output=True,
    ...
    if smoke.returncode != 0:
        raise InstallError("maintenance_bot не импортируется:\n" + smoke.stderr.strip())


def main() -> int:
    parser = argparse.ArgumentParser(description="Install Nyan Wallet maintenance command")
    parser.add_argument("--root", default=".", help="Каталог Telegram-бота")
    args = parser.parse_args()

    root = Path(args.root).expanduser().resolve()
    bot_path = root / "bot.py"
    module_path = root / "maintenance_bot.py"
    ...
    print("✅ Команда /maintenance установлена")
    print("✅ bot.py и maintenance_bot.py прошли py_compile и import smoke-test")
    if bot_backup:
        print(f"📦 Бэкап bot.py: {bot_backup.name}")
    if module_backup:
        print(f"📦 Бэкап maintenance_bot.py: {module_backup.name}")
    print("ℹ️ Перезапусти процесс бота после установки.")
    return 0
